	...
	public class CleanupModerator : MonoBehaviour, IRosMsgReceiveHandler, IAvatarMotionHandler, ITimeIsUpHandler, IGiveUpHandler
	{
		...
		void Awake()
		{
			try
			{
				...
				this.mainPanelController = this.mainMenu != null ? this.mainMenu.GetComponent<PanelMainController>() : null;

				if(this.executionMode == ExecutionMode.FreeCapture) { return; }
		...
		void Start()
		{
			...
			if(this.executionMode == ExecutionMode.FreeCapture)
			{
				this.StartFreeCaptureMode();
				return;
			}
		...
		private void StartFreeCaptureMode()
		{
			Time.timeScale = 1.0f;

			if(this.avatarMotionPlayback != null) { this.avatarMotionPlayback.SetActive(false); }
			if(this.playbackManager     != null) { this.playbackManager    .SetActive(false); }

			CleanupPubMessage cleanupPubMessage = this.GetComponent<CleanupPubMessage>();
			if(cleanupPubMessage != null) { cleanupPubMessage.enabled = false; }

			CleanupSubMessage cleanupSubMessage = this.GetComponent<CleanupSubMessage>();
			if(cleanupSubMessage != null) { cleanupSubMessage.enabled = false; }
			...
			CleanupFreeCaptureMode freeCaptureMode = this.GetComponent<CleanupFreeCaptureMode>();
			if(freeCaptureMode == null)
			{
		...
		public void OnReceiveRosMessage(RosBridge.interactive_cleanup_msgs.msg.InteractiveCleanupMsg interactiveCleanupMsg)
		{
