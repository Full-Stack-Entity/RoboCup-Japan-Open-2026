 ...
 *   handyman::HandymanMsg -> handyman_msgs::msg::HandymanMsg
 */
...
int HandymanTeleopKey::run()
{
  ...
  while (rclcpp::ok()) {
    if (canReceive(kfd)) {
      ...
      // Handle ESC sequences for arrow keys: \x1b [ A/B/C/D
      if (c == '\x1b') {
        // Try to read the next two bytes of the escape sequence
        char seq[2] = {0, 0};
        // Use non-blocking reads for the rest of the sequence
        struct timeval tv;
        fd_set fds;
        tv.tv_sec = 0; tv.tv_usec = 50000;  // 50ms timeout
        FD_ZERO(&fds); FD_SET(kfd, &fds);
