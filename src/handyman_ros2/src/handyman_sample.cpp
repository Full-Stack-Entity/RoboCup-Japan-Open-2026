...
private:
  ...
  bool startMapServer(const std::string &map_file) {
    ...
    stopMapServer();
    // 启动 map_server（lifecycle 节点）
    std::string cmd = "ros2 run nav2_map_server map_server --ros-args -p yaml_filename:=" + map_file + " &";
    system(cmd.c_str());
    std::this_thread::sleep_for(2s);
    // map_server 是 lifecycle 节点，必须 configure + activate 才会发布 /map
    system("ros2 lifecycle set /map_server configure 2>/dev/null || true");
    std::this_thread::sleep_for(1s);
    system("ros2 lifecycle set /map_server activate 2>/dev/null || true");
    std::this_thread::sleep_for(1s);
    // 验证 /map topic 是否有 publisher
    int retries = 10;
    while (retries-- > 0) {
      if (node_->count_publishers("/map") > 0) {
        RCLCPP_INFO(node_->get_logger(), "map_server started and activated successfully");
        return true;
      }
  ...
  bool startAMCL(const geometry_msgs::msg::Pose &initial_pose) {
    ...
    system("pkill -f 'amcl' 2>/dev/null || true");
    std::this_thread::sleep_for(1s);
    // 使用项目自己的 amcl.launch.py，包含 HSR 专用 AMCL 参数
    std::string amcl_cmd;
    try {
      std::string pkg_share = ament_index_cpp::get_package_share_directory("handyman_ros2");
      amcl_cmd = "ros2 launch handyman_ros2 amcl.launch.py &";
    } catch (...) {
      amcl_cmd = "ros2 run nav2_amcl amcl --ros-args -p use_sim_time:=false &";
    }
    system(amcl_cmd.c_str());
    std::this_thread::sleep_for(2s);
    int retries = 10;
    while (retries-- > 0) {
  ...
  bool startNav2() {
    RCLCPP_INFO(node_->get_logger(), "Starting Nav2 with project params...");
    // 使用项目自己的 move_base.launch.py，包含 nav2_params.yaml（DWB + HSR 参数）
    int r = system("ros2 launch handyman_ros2 move_base.launch.py &");
    if (r != 0) RCLCPP_WARN(node_->get_logger(), "Nav2 launch non-zero; may already be running");
    std::this_thread::sleep_for(5s);  // Nav2 启动需要更长时间
...
private:
  ...
  std::vector<std::string> objects = {
    ...
    "soysauce","sauce","filled_ketchup","tumbler","white_cup","pink_cup","empty_ketchup",
    "filled_ketchup","ground_pepper","salt","empty_plastic_bottle","filled_plastic_bottle",
    "clock","toy_car","toy_duck","nursing_bottle","cigarette","hourglass",
