...
def main():
    ...
    process=None
    try:
        for index,case in enumerate(('found','moving','outage','old_success','missing_tf'),1):
            key=f'{index:032x}'
            path=out/(case+'.jsonl')
            with (out/(case+'.console.log')).open('w') as console:
                process=subprocess.Popen([sys.executable,str(scripts/'search_arrival_ros.py'),
                    ...
                    '--output',str(path)],stdout=console,stderr=subprocess.STDOUT)
                ...
                while process.poll() is None and time.monotonic()-began < 25:
                    ...
                    if args.motion_source=='tf':
                        ...
                        if not (case=='outage' and observed is not None):
                    ...
                    if observed is not None and case=='found':
